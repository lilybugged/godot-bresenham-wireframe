...
# and returns true if another part of the mesh (or self-occluding geometry) is closer.
# Custom occlusion test using mesh data.
func is_pixel_occluded(screen_pos: Vector2, expected_depth: float) -> bool:
	...
	if result.has("position"):
		...
		# Use a threshold—here a relative threshold, but adjust as needed.
...
func get_camera_depth(world_point: Vector3) -> float:
	# In Godot 4, use the inverse transform operator (*) to convert to camera local space.
	var cam_inv = camera.global_transform.inverse()
	var local_point = cam_inv * world_point
	# For an orthogonal camera assuming it faces -Z, take the absolute z value.
...
# In your _draw_bresenham_line(), you can now use this version of is_pixel_occluded().
func _draw_bresenham_line(start: Vector2, end: Vector2, base_color: Color, expected_depth_start: float, expected_depth_end: float):
	...
	while true:
		...
		draw_rect(Rect2(current_point, Vector2(1, 1)), pixel_color)
