...
var canEdit = false
var selectedNodes
var dock = preload("res://addons/Wireframe Tool/dock.tscn").instantiate()
var activeTool
var editedMaterial = preload("res://addons/Wireframe Tool/materials/editMat.tres") #material for vertex color preview (unshaded)
var helpDialog = preload("res://addons/Wireframe Tool/helpDialog.tscn").instantiate()
var nameCounter = 0
var edgeSelector = []
...
func _enter_tree():
	dock.wftScript = self
	EditorInterface.get_selection().selection_changed.connect(selectionChanged)
	EditorInterface.get_base_control().add_child(helpDialog)
	add_control_to_dock(DOCK_SLOT_RIGHT_BL, dock)
	activeTool = dock.SELECTION_TOOL_EDGES

func _exit_tree():
	EditorInterface.get_selection().selection_changed.disconnect(selectionChanged)
	EditorInterface.get_base_control().remove_child(helpDialog)
	helpDialog.free()
...
func deleteSurface():
	print("Deleting selected surface...")
	dock.showRenameSurface(false)
	var targetMesh = selectedNodes[dock.getSelTargetMeshId()].get_mesh()
	var ids = surfaceNameToId(targetMesh, dock.getSelTargetSurfaceName())
	remove_surface(targetMesh, int(ids.x))
	#delete meta
	if (targetMesh.has_meta("orgMesh")):
		remove_surface(targetMesh.get_meta("orgMesh"), int(ids.y))
...
func selectionChanged():
	dock.showRenameSurface(false)
	dock.setWarning("")
	selectedNodes = EditorInterface.get_selection().get_selected_nodes()
	edgeSelector.clear()
	vertexSelector = -1
...
func setUnlitShader():
	if (dock.getSelTargetMeshId() ==-1 or isMeshInvalid(selectedNodes[dock.getSelTargetMeshId()])):
		return
...
func setOccluderShader():
	if (dock.getSelTargetMeshId() ==-1 or isMeshInvalid(selectedNodes[dock.getSelTargetMeshId()])):
		push_warning("setOccluderShader: Mesh is invalid.")
		return
	var ids = surfaceNameToId(selectedNodes[dock.getSelTargetMeshId()].get_mesh(), dock.getSelTargetSurfaceName())
	selectedNodes[dock.getSelTargetMeshId()].set_surface_override_material(0,load("res://addons/Wireframe Tool/materials/occluderMat.tres"))
	...
	print(selectedNodes[dock.getSelTargetMeshId()])
...
func remove_surface(mesh: ArrayMesh, surface_index: int):
	...
	for i in range(surface_count):
		...
		if mesh.get_blend_shape_count() > 0:
			blend_shapes = mesh.surface_get_blend_shape_arrays(i)
...
# Usage:
# targetMesh = remove_surface(targetMesh, int(ids.x))
...
func generateWireframeMesh(meshTool, mesh, surfaceId, surfaceName):
	...
	remove_surface(mesh, int(surfaceId))
